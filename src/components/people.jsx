...
export default function People() {
    const [peopleList, setPeopleist] = useState(null);
    const [loading, setLoading] = useState(true);
    const isBigScreen = useMediaQuery({ query: '(min-width: 1024px)' });
  
    useEffect(() => { // Set loading state to true before fetching data
        http.get('/peopleList') // Assuming '/projectList' is the correct endpoint
            .then(res => {
                setPeopleist(res.data);
                setLoading(false);
            })
            .catch(error => {
...
<motion.div className="w-full items-start pl-[1.45rem] md:pl-[2.2rem] lg:pl-[3rem] mb-[1rem] md:mb-[1.45rem] lg:mb-[1rem]"
initial={{opacity: 0}} 
                animate={{opacity: 1}} 
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-center w-full pl-0 md:pl-[2rem] lg:pl-[3rem]  gap-4">
        {loading ? (
          Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className='flex flex-col items-center justify-center mb-[1.56rem] overflow-hidden w-full md:w-[25rem] lg:w-[25rem]'>
              <Loading />
            </div>
          ))
        ):(peopleList && peopleList.map((people, index) => (
